...
const db = drizzle(combinedEnv["DATABASE_URL"]!);
...
async function normalizeRatings() {
  ...
  for (const reviewer of reviewerStats) {
    if (reviewer.reviewCount < 5) {
      console.log(`Skipping ${reviewer.name} - insufficient reviews (${reviewer.reviewCount})`);
    ...
    console.log(`${reviewer.name}: Original avg: ${reviewer.avgRating}, Adjustment: ${adjustment.toFixed(2)}`);
  }
